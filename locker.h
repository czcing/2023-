...
std::string find_password(const std::vector<std::vector<int>>& clues, const std::string& target_hash, int& total_attempts);
std::vector<std::string> find_all_valid_passwords(const std::vector<std::vector<int>>& clues);
// 检查密码是否符合线索（辅助函数）
bool check(const std::string& password,const std::vector<std::vector<int>>& clues);
void password_lock(int &value);
void backtrack(int pos, std::string &password, const std::vector<std::vector<int>>& clues, std::vector<std::string>& valid_passwords);